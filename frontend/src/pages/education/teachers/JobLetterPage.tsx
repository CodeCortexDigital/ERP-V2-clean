...
import { ArrowLeft, Search, Printer, Lock, CheckCircle2, RotateCcw } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-12 p-2">
      ...
      <div className="print:hidden space-y-6">
        ...
        {!selectedTeacher ? (
          <div className="max-w-3xl mx-auto bg-white p-8 rounded-3xl border border-slate-100 shadow-sm text-center space-y-6 mt-12">
            ...
            <div className="space-y-1">
              <h2 className="text-lg font-bold text-slate-800">Generate Job Letter</h2>
              <p className="text-xs text-slate-500">Search an active employee by name, ID, or designation to print their job letter.</p>
            </div>
            ...
            <div className="relative max-w-md mx-auto">
              <input
                ...
                className="w-full pl-4 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 font-medium text-slate-700"
              />
              <button className="absolute right-2 top-1.5 p-2 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-colors">
                <Search className="w-4 h-4" />
              </button>
            </div>

            {/* List results */}
            <div className="max-w-md mx-auto max-h-48 overflow-y-auto space-y-1.5 border border-slate-100 rounded-2xl p-2 bg-slate-50/50">
              {loading ? (
                <p className="text-xs text-slate-400 py-3">Loading employees list...</p>
              ) : filteredTeachers.length === 0 ? (
                <p className="text-xs text-slate-400 py-3">No matching employees found.</p>
              ) : (
                filteredTeachers.map(t => (
                  <button
                    key={t.id}
                    onClick={() => setSelectedTeacher(t)}
                    className="w-full text-left p-3 rounded-xl hover:bg-white hover:shadow-xs transition border border-transparent hover:border-slate-150 flex items-center justify-between text-xs font-bold text-slate-700"
                  >
                    <span>{t.full_name}</span>
                    <span className="text-[10px] text-purple-600 font-mono bg-purple-50 px-2 py-0.5 rounded-lg">{t.employee_id || 'Staff'}</span>
                  </button>
                ))
              )}
            </div>
